...
function AssetStatusesPageContent() {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      const deletePromises = assetStatuses.map(async (status, index) => {
        const id = status.get('id');
        if (!id) {
          errorCount++;
          errors.push(`حالة #${index + 1} بدون معرف`);
      ...
      if (errorCount > 0) {
        const errorMessage = errors.slice(0, 3).join('، ');
        const moreErrors = errors.length > 3 ? ` و ${errors.length - 3} خطأ آخر` : '';
        showWarning(`تم حذف ${successCount} من ${assetStatuses.length} حالة بنجاح، فشل: ${errorCount}. ${errorMessage}${moreErrors}`, 8000);
      } else {
        showSuccess(`تم حذف جميع ${successCount} حالة بنجاح`);
      }
