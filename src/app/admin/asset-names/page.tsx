...
function AssetNamesPageContent() {
  ...
  const handleDelete = async (assetName: BaseModel) => {
    const id = assetName.get('id');
    if (!id) return;
    if (!confirm(`هل أنت متأكد من حذف ${assetName.get('name')}؟`)) return;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-6">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-4xl font-bold bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 bg-clip-text text-transparent">أسماء الأصول</h1>
                <p className="text-slate-600 text-base mt-1 font-medium">إدارة وإضافة أسماء الأصول في النظام</p>
              </div>
            </div>
          </div>
          <Button
